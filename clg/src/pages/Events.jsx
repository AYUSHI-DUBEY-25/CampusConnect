...
//     try {
//       const { data } = await axios.get(
//         "http://localhost:8080/api/v1/event/event-count"
//       );
//       setTotal(data.total);
//     } catch (error) {
//       console.log(error);
...
//       setLoading(true);
//       const { data } = await axios.get(
//         `http://localhost:8080/api/v1/event/event-list/${page}`
//       );
//       setLoading(false);
...
//         "http://localhost:8080/api/v1/event/event-filters",
//         { checked, radio }
//       );
//       setEvents(data.event);
//     } catch (error) {
...
//   // Add to cart helper (used by card and modal)
//  // Add to cart helper (merge + cap)
// const addItemToCart = (item) => {
//   // item must include quantity (Number)
//   if (!auth?.user) {
...
//     // Option A: refuse and tell user how many left
//     toast.error(`Only ${available - existingQty} more tickets available`);
//     return false;

//     // Option B (alternate behaviour): cap to available and update
//     // const newQty = available;
//     // ...
//   }

//   // build updated cart array (merge)
//   let updatedCart = [];
//   if (existing) {
...
//   // persist to localStorage (keep your current storage strategy)
//   setCart(updatedCart);
//   try {
//     // prefer per-user key if you have auth
...
//   // compute how many of this event are already in the cart
...
//     // default quantity = 1 when adding from card
...
//   // Add to cart from modal, using ticketQty
//   const addFromModal = () => {
...
//                 <img
//                   src={`http://localhost:8080/api/v1/event/event-photo/${selectedEvent._id}`}
...
const Events = () => {
  ...
  useEffect(() => {
    if (selectedEvent) setTicketQty(1);
  }, [selectedEvent]);
  ...
  useEffect(() => {
    if (checked.length === 0 && radio.length === 0) getAllEvents();
    else filterEvent();
    // eslint-disable-next-line
  }, [checked, radio]);
  ...
  const addItemToCart = (item) => {
    ...
    if (existingQty + qtyToAdd > available) {
      toast.error(`Only ${available - existingQty} more tickets available`);
      return false;
    }
    ...
    if (existing) {
      updatedCart = cart.map((c) =>
        c._id === item._id
          ? { ...c, quantity: existingQty + qtyToAdd }
          : c
      );
    } else {
      updatedCart = [...cart, { ...item, quantity: qtyToAdd }];
  ...
  const EventCard = ({ p }) => {
    ...
    const canAddMore = p.ticketsAvailable - inCartQty > 0;
    ...
    return (
      <div className="event-card">
        ...
        <button
          onClick={() => addItemToCart({ ...p, quantity: 1 })}
          disabled={!canAddMore}
          className="btn btn-dark mt-2"
        >
          {!canAddMore ? "Sold Out" : "Add to Cart"}
        </button>

        {inCartQty > 0 && <small>In cart: {inCartQty}</small>}
      </div>
    );
  };
  ...
  const addFromModal = () => {
    if (!selectedEvent) return;
    const ok = addItemToCart({ ...selectedEvent, quantity: ticketQty });
        ...
        <div className="modal fade show d-block bg-dark bg-opacity-75">
          <div className="modal-dialog modal-lg">
            <div className="modal-content p-3">
              ...
              <p>Tickets Available: {selectedEvent.ticketsAvailable}</p>
              ...
              <div className="mt-3">
                ...
                <button className="btn btn-primary" onClick={addFromModal}>
                  Add to Cart
                </button>
              </div>
            </div>
